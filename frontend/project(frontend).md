...
- [ ] **Задача:** **Исправить отображение уроков:** Убедиться, что контент урока (`lesson.content`) корректно загружается и отображается. Исправить ошибку с дублированием уроков в списке.
- [ ] **Задача:** **Реализовать функционал тестов (Quizzes):**
    - [ ] Создать страницу `app/(main)/lessons/[lessonId]/quiz/page.tsx` для прохождения теста.
    - [ ] Разработать компоненты для отображения вопросов (`QuestionCard`) и вариантов ответов.
    - [ ] Создать API-хуки для получения данных теста (`useGetQuizByLessonIdQuery`) и отправки результатов (`useSubmitQuizMutation`).
    - [ ] Реализовать логику проверки ответов и отображения результатов.
- [ ] **Задача:** **Добавить навигацию к тестам:** Встроить кнопку или ссылку для перехода к тесту со страницы урока.
...
- [ ] **Задача:** Добавить в `components/ui` продвинутые компоненты: `AlertDialog`, `Sheet`, `Toast`.
...
1.  **Завершить реализацию страницы курса:** Исправить баг с дублированием уроков и покрыть компоненты тестами (**Шаг 5**).
2.  **Расширить UI-кит:** Добавить недостающие UI-компоненты по мере необходимости (**Шаг 6**).
3.  **Внедрить глобальное состояние:** При необходимости добавить `Zustand` для управления состоянием.
