...
export function QuizViewEnhanced({ lessonId }: QuizViewEnhancedProps) {
  ...
  const submitMutation = useSubmitQuizMutation(quiz?.id ?? '', {
    onSuccess: data => {
      ...
      // queryClient.invalidateQueries({ queryKey: ['get subject by id', quiz?.lessonId] })
    },
    onError: error => {
    ...
    return (
      <Card className="mx-auto max-w-4xl">
        ...
        <CardContent className="space-y-4 text-center">
          ...
          <div className="mt-4">
            <p className="text-lg">
              {isPassed ? 'Поздравляем! Вы успешно прошли тест.' : 'Попробуйте пройти тест еще раз.'}
            </p>
          </div>
        </CardContent>
